...
pub fn inv_sub_bytes_fhe(
    state: &mut [Ciphertext],
    sbox_inv_msb: &BivariateLookupTableOwned,
    sbox_inv_lsb: &BivariateLookupTableOwned,
    sk: &ServerKey,
) {
    let start = Instant::now();
    assert!(state.len() % 2 == 0);
    ...
    state.par_iter_mut().enumerate().for_each(|(i, elem)| {
        if i % 2 == 0 {
            *elem = sk.apply_lookup_table_bivariate(&tmp[i], &tmp[i + 1], sbox_inv_msb);
        } else {
            *elem = sk.apply_lookup_table_bivariate(&tmp[i - 1], &tmp[i], sbox_inv_lsb);
        }
    });
...
#[inline]
fn inv_shift_rows_fhe(state: &mut [Ciphertext]) {
    let start = Instant::now();
    let tmp = state.to_vec();
    ...
    state[2 * 7 + 1] = tmp[2 * 11 + 1].clone();

    // col. 2
    state[2 * 8] = tmp[2 * 8].clone();
    state[2 * 8 + 1] = tmp[2 * 8 + 1].clone();
    state[2 * 9] = tmp[2 * 5].clone();
    state[2 * 9 + 1] = tmp[2 * 5 + 1].clone();
    state[2 * 10] = tmp[2 * 2].clone();
    state[2 * 10 + 1] = tmp[2 * 2 + 1].clone();
    state[2 * 11] = tmp[2 * 15].clone();
    state[2 * 11 + 1] = tmp[2 * 15 + 1].clone();

    // col. 3
    state[2 * 12] = tmp[2 * 12].clone();
    state[2 * 12 + 1] = tmp[2 * 12 + 1].clone();
    state[2 * 13] = tmp[2 * 9].clone();
    state[2 * 13 + 1] = tmp[2 * 9 + 1].clone();
    state[2 * 14] = tmp[2 * 6].clone();
    state[2 * 14 + 1] = tmp[2 * 6 + 1].clone();
    state[2 * 15] = tmp[2 * 3].clone();
    state[2 * 15 + 1] = tmp[2 * 3 + 1].clone();
...
fn inv_mix_columns_fhe(
    state: &mut [Ciphertext],
    gmul9_msb: &BivariateLookupTableOwned,
    gmul9_lsb: &BivariateLookupTableOwned,
    gmulb_msb: &BivariateLookupTableOwned,
    gmulb_lsb: &BivariateLookupTableOwned,
    gmuld_msb: &BivariateLookupTableOwned,
    gmuld_lsb: &BivariateLookupTableOwned,
    gmule_msb: &BivariateLookupTableOwned,
    gmule_lsb: &BivariateLookupTableOwned,
    sk: &ServerKey,
) {
    let start = Instant::now();
    assert!(state.len() == 32);

    let g9_state = lut_state(state, gmul9_msb, gmul9_lsb, sk);
    let gb_state = lut_state(state, gmulb_msb, gmulb_lsb, sk);
    let gd_state = lut_state(state, gmuld_msb, gmuld_lsb, sk);
    let ge_state = lut_state(state, gmule_msb, gmule_lsb, sk);

    let mut binding: Vec<Ciphertext> = (0..32).map(|_| sk.create_trivial(0)).collect();
    let g9_gb_xor = binding.as_mut_slice();
    let g9_idx = vec![6, 0, 2, 4];
    let gb_idx = vec![2, 4, 6, 0];
    parallel_xor(g9_gb_xor, &g9_state, &gb_state, &g9_idx, &gb_idx, sk);

    let mut binding: Vec<Ciphertext> = (0..32).map(|_| sk.create_trivial(0)).collect();
    let gd_ge_xor = binding.as_mut_slice();
    let gd_idx = vec![4, 6, 0, 2];
    let ge_idx = vec![0, 2, 4, 6];
...
pub fn decrypt_block_fhe(
    ...
    println!("generate_bivariate_tables");
    let (inv_sbox_msb, inv_sbox_lsb) = gen_tbl(&SBOX_INV, &sk);
    let (gmul9_msb, gmul9_lsb) = gen_tbl(&GMUL9, &sk);
    let (gmulb_msb, gmulb_lsb) = gen_tbl(&GMULB, &sk);
    let (gmuld_msb, gmuld_lsb) = gen_tbl(&GMULD, &sk);
    let (gmule_msb, gmule_lsb) = gen_tbl(&GMULE, &sk);
    ...
    for i in 1..=iter {
        ...
        print_hex_nibble_fhe("iinput", 0, &state_ck, &ck);
        add_round_key_fhe(
            &mut state_ck,
            &xk_ck[2 * KEYSIZE * ROUNDS..2 * ROUNDKEYSIZE],
            &sk,
        );
        print_hex_nibble_fhe("ik_sch", 0, &state_ck, &ck);

        for round in (1..ROUNDS).rev() {
            inv_shift_rows_fhe(&mut state_ck);
            print_hex_nibble_fhe("is_row", round, &state_ck, &ck);

            inv_sub_bytes_fhe(&mut state_ck, &inv_sbox_msb, &inv_sbox_lsb, &sk);
            print_hex_nibble_fhe("is_box", round, &state_ck, &ck);

            add_round_key_fhe(
                &mut state_ck,
                &xk_ck[round * 2 * KEYSIZE..(round + 1) * 2 * KEYSIZE],
                &sk,
            );
            print_hex_nibble_fhe("ik_sch", round, &state_ck, &ck);
            ...
            print_hex_nibble_fhe("ik_add", round, &state_ck, &ck);
        }

        inv_shift_rows_fhe(&mut state_ck);
        print_hex_nibble_fhe("is_row", 0, &state_ck, &ck);

        inv_sub_bytes_fhe(&mut state_ck, &inv_sbox_msb, &inv_sbox_lsb, &sk);
        print_hex_nibble_fhe("is_box", 0, &state_ck, &ck);
